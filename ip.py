...
from core import KNXException, ValueCache, E_NO_ERROR
from helper import int_to_array, ip_to_array
from gatewayscanner import GatewayScanner
...
class KNXIPTunnel():
    ...
    def disconnect(self):
        ...
        if self.connected and self.channel:
            ...
            print("Disconnecting KNX/IP tunnel...")
            ...
            print("Disconnect - no connection, nothing to do")
    ...
    def group_read(self, addr, use_cache=True, timeout=1):
        ...
        if use_cache:
            ...
            if res:
                logging.debug(
                    "Getting value of group address %s from cache: %s...", addr, res)
                return res
